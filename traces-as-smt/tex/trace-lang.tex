...
\inferrule[Process Queue Movement]{
  ...
  (\epsnd\ \qset\ ((\dst\ \src)\ (\dst_0\ \src_0)\ \ldots\ \bot)\ \status) \reduce{q}(\epsnd_p\ \qset_p\ ((\dst_0\ \src_0)\ \ldots\ \bot)\ \status)
}
\end{mathpar}}
...
receive (\rcv), and wait. The queue movement (\movebot) is either a special symbol indicating no movement or a list of move commands.
The move list (\movelist) consists of several pairs of endpoints where the first and second endpoints are the destination and source endpoints respectively.
Each pair in the move list refers to a queue movement such that the head item of the pending sends in the source endpoint should be moved to the send queue in the destination endpoint.
...
(\traceentry).  A trace entry is a pair of consisting of a program location (\ploc) and a list of queue movement. An example
...
a list of queue movement with which to move commands among the queues and to match the send action with the same index in the send queue as that of this receive action in the receive queue, in order to resolve any
...
\epsnd\ (a store mapping any given destination endpoint \dst\ to another store which then maps any given source endpoint \src\ to a list of pairs where each pair is an action ID \aid\ and a value $v$),
\eprcv\ (a store mapping any given destination endpoint \dst\ to a list of pairs where each pair is an action ID \aid\ and a variable $x$) and \qset (a store mapping any given destination endpoint \dst\ to a list of pairs where each pair is an action IF \aid\ and a value $v$).  Intuitively, \epsnd\, \qset\ and \eprcv\ are endpoint queues tracking actions with associated values (sends) or variables (receives). Both \epsnd\ and \qset\ store the sends. Their difference is \epsnd\ records the source and destination endpoints, but \qset\ only records the destination endpoint. \eprcv\ and \qset\ has the similar structure so that they can be applied to store and find matched send and receive actions in machine steps.
...
\emph{Machine Step} inference matches any \textit{mstate} with
a thread sitting at a command
\[
(\thread_0~\ldots~([\ploc_0~\cmd_0]~[\ploc_1~\cmd_1]~[\ploc_2~\cmd_2]~\ldots)~\thread_2~\ldots))
\]
and having the next entry in the trace match on that threads location
$([\ploc_0~\movebot_0]~\traceentry_1~\ldots)$.  A match on the inference
...
more reductions apply (as indicated by the asterisk). Note the queue reductions perform the queue movement such that all send actions are moved to the destination send queue, and the matching can be processed in the expression reduction. At this point, send and receive actions are matched in the wait expression.  The inference for \textit{Machine Step} on the other commands is defined similarly.

The queue reduction for each command of the queue movement list in the trace entry is given in \figref{fig:queue}. The definition of \textit{qstate}  four-tuple is presented in the evaluation syntax in \figref{fig:expr:stx}(b). The symbol $\bot$ in the queue movement \movebot\ indicates that no more queue movement follows. The pending sends \epsnd\ stores all the Sndi commands for which the expression reductions were performed. The send queue \qset\ then stores those Sndi commands indexed by the destination endpoint in the queue reductions. In the mean time, they are also removed from \epsnd\ in the reduction process. Each reduction step processes the first pair of the queue movement list and reduction steps follow until $\bot$ is shown.
...
applies the ``op'' to the left and right operands. The function \emph{getMarkRemove} is described in the later paragraph. The other helper functions are defined below:
\begin{eqnarray*}
...
only.  The function of the other rule for the wait command are more involved. The \emph{Waiti
(RCVI)} inference performs the actual update by parallel searching the Rcvi command in \eprcv\ and Sndi command in \qset\ (defined in helper function \emph{getMarkRemove}).
The only matched Sndi command is the one that has the same index number with that of the Rcvi command. The sends and receives with the same index number (less than the index number of the Sndi and Rcvi commands) in \qset\ and \eprcv\ respectively will then be considered as matched by marking the value of each send in $h$ with $\bot$. This is to avoid the ``reassignment" of variables for other matching process. After this step, \aidmap\ should be updated by removing the action IDs for both the Sndi and Rcvi commands.
...
\[(h\ \eta\ \aidmap\ \epsnd\ \eprcv\ \qset\ \cfgnt{ctp}\ \trace\ \status\ k) \reduceK{m} (h_p\ \eta_p\ \aidmap_p\ \epsnd_p\ \eprcv_p\ \qset_p\ \cfgnt{ctp}_p\ \trace_p\ \status_p\ k_p)\]
such that
\[
\begin{array}{llll}
\aidmap_p = ()                    & \wedge & \forall \dst, \forall \src, \epsnd_p(\dst)(\src) = () & \wedge \\
\forall \dst, \eprcv_p(\dst) = () & \wedge & \forall \dst, \qset_p(\dst) = ()                               & \wedge\\
\cfgnt{ctp}_p\ = (()\ \ldots)     & \wedge & \trace_p\  = ()                                          & \wedge \\
\status_p\ \prec \cfgt{error}     & \wedge & k_p = \cfgt{ret}
\end{array}
\]
